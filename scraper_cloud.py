...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        REVIEW_KEYWORDS = ["review", "feedback", "rating", "customer"]
        all_review_notifs = [
            n for n in all_notifs
            if any(kw in n.get("title", "").lower() for kw in REVIEW_KEYWORDS)
        ]
        print(f"  {len(all_review_notifs)} review-type notifications in history")

        known_branches = {}   # branch_id → {site, url}

        # First: prioritise TODAY's review notifications to get fresh branches
        today_review_notifs = [
            n for n in today_notifs
            if any(kw in n.get("title", "").lower() for kw in REVIEW_KEYWORDS)
        ]
        print(f"  {len(today_review_notifs)} review-type notifications TODAY")

        for n in (today_review_notifs + [x for x in all_review_notifs if x not in today_review_notifs]):
            dr = await api_get(page, f"{NOTIF_API}/{n['id']}", token)
            if not dr.get("ok"):
                continue
            ...
            for res in resources:
                ...
                sname = res.get("title", "Unknown")
                if bid and bid not in known_branches:
                    full_link = link if link.startswith("http") else f"{BASE_URL}{link}"
                    known_branches[bid] = {
                        "site": sname, "url": full_link,
                        "from_today": n in today_review_notifs
                    }
                    print(f"    + {sname} (branchId={bid})"
                          f"{' ← TODAY' if n in today_review_notifs else ''}")
