...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        yesterday_notifs = [n for n in all_notifs if notif_uk_date(n.get("timestamp", "")) == yesterday]
        ...
        for branch_id, info in known_branches.items():
            ...
            today_reviews = [
                ...
                if to_uk_date(r.get("created_at", "")) == today
            ]
    ...
    # Store today's reviews under today's UK date
    history[today] = {
        "scraped_date": today,
        "last_updated": now_utc,
        "cancelled_orders": [],   # today's cancellations not available yet (API lag)
        "reviews":        reviews,
    }

    # Store yesterday's cancellations under YESTERDAY'S date (where they belong)
    if yesterday not in history:
        history[yesterday] = {"scraped_date": yesterday, "reviews": []}
    history[yesterday]["cancelled_orders"] = cancelled
    history[yesterday]["last_updated"]     = now_utc

    OUT_FILE.write_text(json.dumps(history, indent=2))
    print(f"\n✓ Done — {len(cancelled)} cancelled → data.json[{yesterday}], "
