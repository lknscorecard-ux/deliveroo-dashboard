...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        # Return to home so session is stable
        try:
            await page.goto(HOME_URL, wait_until="domcontentloaded", timeout=15000)
        except Exception:
            pass
        ...
        for branch_id, info in known_branches.items():
            ...
            # Try intercepted data first (from Strategy D) — no extra API call needed
            raw_reviews = intercepted_review_data.get(branch_id)

            if raw_reviews is None:
                # Try both API paths: /api/restaurants and /api-gw/restaurants
                for api_url in [
                    ...
                    if result.get("ok"):
                        rd = result.get("data", {})
                        raw_reviews = (rd.get("reviews") or rd.get("data") or
                                       (rd if isinstance(rd, list) else []))
                        raw_reviews = raw_reviews if isinstance(raw_reviews, list) else []
                        if raw_reviews:
                            print(f"  {site_name}: {len(raw_reviews)} review(s) via {api_url[-40:]}")
                            break
                    elif not result.get("ok") and result.get("status") not in [401, 403, 404]:
                        # Session drop — refresh and retry once
                        try:
                            await page.goto(HOME_URL, wait_until="domcontentloaded", timeout=15000)
                        except Exception:
                            pass
            ...
            if not raw_reviews:
                print(f"  {site_name} (bid={branch_id}): no reviews returned")
                continue
            ...
            if not today_reviews:
                print(f"  {site_name} (bid={branch_id}): {len(raw_reviews)} total but 0 today")
                continue

            print(f"  {site_name} (bid={branch_id}): {len(today_reviews)} review(s) TODAY")
