...
async def login(page):
    ...
    if "login" not in page.url and "signin" not in page.url and "auth" not in page.url:
        print("✓ Already logged in.")
        return True

    print(f"Login page detected: {page.url}")

    # Fill email
    try:
        await page.wait_for_selector('input[type="email"], input[name="email"], input[id="email"]', timeout=8000)
        await page.fill('input[type="email"], input[name="email"], input[id="email"]', DELIVEROO_EMAIL)
    ...
    # Click Continue/Next if present (some flows split email/password)
    for btn_text in ["Continue", "Next", "Sign in with email"]:
        try:
    ...
    try:
        ...
        await page.fill('input[type="password"]', DELIVEROO_PASSWORD)
        await page.wait_for_timeout(500)
    except Exception as e:
    ...
    # Submit
    for btn_text in ["Sign in", "Log in", "Login", "Continue", "Submit"]:
        try:
            btn = page.locator(f"button:has-text('{btn_text}')").first
            if await btn.is_visible(timeout=1500):
                await btn.click()
    ...
    # Wait for redirect to partner hub
    try:
        await page.wait_for_url("**/partner-hub.deliveroo.com/**", timeout=20000)
    except Exception:
        pass
...
async def main():
    if not DELIVEROO_EMAIL or not DELIVEROO_PASSWORD:
        print("ERROR: DELIVEROO_EMAIL and DELIVEROO_PASSWORD env vars must be set.")
        raise SystemExit(1)
    ...
    async with async_playwright() as pw:
        ...
        # Navigate to home page for correct org context
        try:
