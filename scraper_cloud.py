...
async def main():
    ...
    # Cancelled orders come from the notification API (oldest-first, hard limit 200).
    # Today's notifications are always at position 201+ so we can't reach them.
    # Yesterday's notifications sit within the 200 and are fully captured.
    # Reviews come from a separate per-branch API with no lag → use today.
    print(f"Cancelled orders target: {yesterday} (notification API — yesterday only)"
          f"\nReviews target:          {today} (branch reviews API — real-time)")
    ...
    async with async_playwright() as pw:
        ...
        # Notification API is oldest-first, hard-capped at 200.
        # Yesterday's notifications are within the 200; today's are at 201+.
        yesterday_notifs = [n for n in all_notifs if yesterday in n.get("timestamp", "")]
        print(f"  Yesterday ({yesterday}): {len(yesterday_notifs)} notifications (for cancelled orders)")
        for n in yesterday_notifs:
            print(f"    [{n['timestamp']}] {n['title']}")

        # ── 5. Cancelled orders — yesterday only (notification API limitation) ─
        cancelled = []
        for n in yesterday_notifs:
            title = n.get("title", "")
        ...
        for branch_id, info in known_branches.items():
            ...
            # Reviews: use today (branch reviews API has no lag, real-time).
            # BST midnight edge: 00:xx BST = 23:xx UTC previous day → include those too.
            today_reviews = [
            ...
            print(f"  {site_name} (branchId={branch_id}): {len(today_reviews)} review(s) today")
    ...
    history[today] = {
        ...
        "cancelled_orders_date":   yesterday,   # cancelled orders are from yesterday (API limitation)
        "cancelled_orders":        cancelled,
        "reviews":                 reviews,     # reviews are from today (real-time)
    }
    OUT_FILE.write_text(json.dumps(history, indent=2))
    print(f"\n✓ Done — {len(cancelled)} cancelled (from {yesterday}), "
          f"{len(reviews)} reviews (from {today}) → data.json[{today}]")
