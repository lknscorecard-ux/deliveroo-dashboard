"""
Deliveroo Partner Hub — Cloud Scraper v7
...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        if not result.get("ok"):
            print(f"ERROR fetching notifications: {result}")
            raise SystemExit(1)
        ...
        print(f"Total: {len(all_notifs)} | Yesterday: {len(yesterday_notifs)}")
        for n in yesterday_notifs:
            print(f"  [{n['timestamp']}] {n['title']}")

        # ── 4. Process notifications ──────────────────────────────────────────
        cancelled = []
        review_tasks = []   # {site, branch_id, count}
            ...
            if "cancelled" in title.lower() or "auto-rejected" in title.lower():
                ...
                for r in resources:
                    ...
                    if branch_id:
                        ...
                        print(f"    {site_name} (branchId={branch_id}): {count} new review(s)")
        ...
        for task in review_tasks:
            ...
            print(f"  API returned {len(raw_reviews)} total reviews, taking {count} most recent")

            for rev in raw_reviews[:count]:
