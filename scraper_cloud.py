...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        # Reviews: look at ALL recent "Customer reviews" notifications (last 10)
        # because multiple orgs send separate batches at different times
        all_review_notifs = [n for n in all_notifs
                             if "review" in n.get("title", "").lower()][:10]
        print(f"\n  Found {len(all_review_notifs)} recent review notification(s) across all orgs")
