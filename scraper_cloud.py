...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        # ── 3. Fetch ALL notifications (paginate until 2 days before yesterday) ──
        ...
        while True:
            ...
            print(f"  Page {page_num}: {len(batch)} notifications (total: {len(all_notifs)}, oldest: {oldest_ts})")

            # Stop once we've passed 2 days ago (covers all of yesterday regardless of timezone)
            if oldest_ts and oldest_ts[:10] < two_days_ago:
                break

            # Find cursor for next page — try common field names
            last = batch[-1]
            cursor = (last.get("id") or last.get("_id") or last.get("notificationId")
                      or last.get("alertId") or last.get("notification_id") or "")
            if not cursor:
                print(f"  No pagination cursor found in last notification keys: {list(last.keys())}")
                break
            if len(batch) < 200:
                break
            url = f"{NOTIF_API}?limit=200&starting_after={cursor}"
