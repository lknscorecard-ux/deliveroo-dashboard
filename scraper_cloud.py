...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        # ── 2b. Intercept notifications URLs called by the Partner Hub UI ─────
        intercepted_urls = set()
        async def on_response(response):
            if "notifications" in response.url and "alerts" in response.url:
                intercepted_urls.add(response.url)
        page.on("response", on_response)

        # Navigate to home and click the bell to trigger the UI's own API calls
        try:
            bell = page.locator('[data-testid="notification-bell"], [aria-label*="notification" i], button:has(svg)').first
            if await bell.is_visible(timeout=3000):
                await bell.click()
        ...
        if intercepted_urls:
            print("Intercepted notification URLs from UI:")
            for u in sorted(intercepted_urls):
                print(f"  {u}")
