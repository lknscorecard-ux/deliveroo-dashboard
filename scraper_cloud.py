...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        for oid in sorted(org_ids):
            ...
            # Try newest-first to capture today's notifications;
            # fall back to default if sort param not supported
            r = await api_get(page, f"{NOTIF_API}?limit=200&sort=desc", token)
            if not r.get("ok") or not isinstance(r.get("data"), list):
                r = await api_get(page, f"{NOTIF_API}?limit=200&order=desc", token)
            if not r.get("ok") or not isinstance(r.get("data"), list):
